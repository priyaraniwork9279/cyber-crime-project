...
def get_df():
    if "data" not in st.session_state:
        st.session_state.data = pd.DataFrame(DEFAULT_ROWS, columns=COLS)
        st.session_state.data["Incident_Date"] = pd.to_datetime(st.session_state.data["Incident_Date"])
        st.session_state.data["Loss_Amount"] = st.session_state.data["Loss_Amount"].astype(int)
        st.session_state.data["Victim_Age"] = st.session_state.data["Victim_Age"].astype(int)
...
def show_login():
    ...
    with col2:
        ...
        <div style='font-size:1.2rem;font-weight:900;color:#fff;margin:8px 0 4px;'>ADMIN LOGIN</div>
        <div style='font-size:0.65rem;color:#1e5070;letter-spacing:2px;margin-bottom:20px;'>CYBERSHIELD CONTROL PANEL</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if st.button("🚀 LOGIN KARO", use_container_width=True):
        ...
        st.markdown("""
        <div style='text-align:center;margin-top:14px;font-size:0.72rem;color:#1e4060;'>
        Username: <b style='color:#3a7fa8;'>admin</b> &nbsp;|&nbsp; Password: <b style='color:#3a7fa8;'>cyber123</b>
        </div>
        """, unsafe_allow_html=True)
...
def show_admin():
    ...
    with st.sidebar:
        ...
        <div style='font-size:1.1rem;color:#00c8ff;font-weight:900;'>🛡️ ADMIN PANEL</div>
        <div style='font-size:0.6rem;color:#1e4060;letter-spacing:2px;margin-top:2px;'>CYBERSHIELD CONTROL</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    # ── ADD ──────────────────────────────────────────────────────────────────
    if "➕" in menu:
        st.markdown("<h2 style='color:#fff;'>➕ Naya Crime Case Add Karo</h2>", unsafe_allow_html=True)
        nid = next_id()
        st.info(f"🆔 Naya Case ID: **{nid}** (automatic)")

        c1, c2 = st.columns(2)
        with c1:
            d   = st.date_input("📅 Tarikh", value=date.today())
            ft  = st.selectbox("⚠️ Fraud Type", ["UPI Phishing","OTP Fraud","Investment Scam","Social Media Fraud","Email Phishing","Other"])
            amt = st.number_input("💸 Loss Amount (₹)", min_value=0, step=500, value=10000)
        with c2:
            reg = st.selectbox("📍 District", ["Patna","Gaya","Muzaffarpur","Darbhanga","Bhagalpur","Ara","Nalanda","Siwan","Begusarai","Hajipur","Samastipur","Other"])
            st8 = st.text_input("🏛️ State", value="Bihar")
            age = st.number_input("👤 Victim Age", min_value=1, max_value=100, value=30)
            sts = st.selectbox("📋 Status", ["Under Investigation","Resolved","Closed"])

        st.markdown("<br>", unsafe_allow_html=True)
        if st.button("✅ SAVE KARO", use_container_width=True):
            new_row = pd.DataFrame([[nid, pd.to_datetime(d), ft, amt, reg, st8, age, sts]], columns=COLS)
            updated = pd.concat([df, new_row], ignore_index=True)
            save_df(updated)
            st.success(f"✅ Case {nid} add ho gaya!")
            st.balloons()
    ...
    # ── EDIT ─────────────────────────────────────────────────────────────────
        ...
        df = get_df()
        sel = st.selectbox("🔍 Case ID choose karo", df["Crime_ID"].tolist())
        row = df[df["Crime_ID"] == sel].iloc[0]

        st.markdown(f"<div style='background:#080f1e;border:1px solid #0d2a42;border-left:3px solid #00c8ff;border-radius:8px;padding:12px 16px;margin-bottom:16px;color:#6a9ab8;'>Editing: <b style='color:#00c8ff;'>{sel}</b> — {row['Fraud_Type']} — {row['Region']} — ₹{int(row['Loss_Amount']):,}</div>", unsafe_allow_html=True)

        c1, c2 = st.columns(2)
        with c1:
            nd  = st.date_input("📅 Tarikh", value=pd.to_datetime(row["Incident_Date"]).date())
            nft = st.selectbox("⚠️ Fraud Type", ["UPI Phishing","OTP Fraud","Investment Scam","Social Media Fraud","Email Phishing","Other"],
            ...
            na  = st.number_input("💸 Loss Amount (₹)", min_value=0, step=500, value=int(row["Loss_Amount"]))
        with c2:
            ...
            nst = st.selectbox("📋 Status", ["Under Investigation","Resolved","Closed"],
        ...
        if st.button("💾 UPDATE KARO", use_container_width=True):
            ...
            df.at[i,"Status"]        = nst
            save_df(df)
            st.success(f"✅ Case {sel} update ho gaya!")

    # ── DELETE ────────────────────────────────────────────────────────────────
    elif "🗑️" in menu:
        st.markdown("<h2 style='color:#fff;'>🗑️ Case Delete Karo</h2>", unsafe_allow_html=True)
        df = get_df()
        st.warning("⚠️ Delete karne ke baad case wapas nahi aayega!")

        sel = st.selectbox("🔍 Case ID choose karo", df["Crime_ID"].tolist())
        row = df[df["Crime_ID"] == sel].iloc[0]

        st.markdown(f"""
        <div style='background:#1a0505;border:1px solid #cc0000;border-radius:8px;padding:14px 18px;margin:12px 0;'>
        <b style='color:#ef4444;font-size:1rem;'>{sel}</b><br>
        <span style='color:#6a9ab8;font-size:0.85rem;'>
        ⚠️ {row['Fraud_Type']} &nbsp;|&nbsp; 📍 {row['Region']} &nbsp;|&nbsp; 💸 ₹{int(row['Loss_Amount']):,} &nbsp;|&nbsp; 📋 {row['Status']}
        </span></div>
        """, unsafe_allow_html=True)

        confirm = st.checkbox(f"Haan, main {sel} permanently delete karna chahta/chahti hun")
        if confirm:
            if st.button("🗑️ DELETE KARO", use_container_width=True):
                updated = df[df["Crime_ID"] != sel].reset_index(drop=True)
                save_df(updated)
                st.success(f"✅ Case {sel} delete ho gaya!")
                st.rerun()

        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("<b style='color:#3a7fa8;'>📋 Saare Records:</b>", unsafe_allow_html=True)
        st.dataframe(df[["Crime_ID","Fraud_Type","Region","Loss_Amount","Status"]], use_container_width=True, hide_index=True)

    # ── DASHBOARD ─────────────────────────────────────────────────────────────
    elif "📊" in menu:
        st.markdown("<h2 style='color:#fff;'>📊 Admin Dashboard</h2>", unsafe_allow_html=True)
        df = get_df()

        n = len(df)
        tl = df["Loss_Amount"].sum()
        ur = round(len(df[df["Fraud_Type"]=="UPI Phishing"])/max(n,1)*100,1)
        re = len(df[df["Status"]=="Resolved"])

        c1,c2,c3,c4 = st.columns(4)
        c1.metric("🗂️ Total Cases", n)
        c2.metric("💸 Total Loss", f"₹{tl/100000:.1f}L")
        c3.metric("📶 UPI Rate", f"{ur}%")
        c4.metric("✅ Resolved", re)
        ...
        st.markdown("<b style='color:#3a7fa8;'>📋 All Records:</b>", unsafe_allow_html=True)
...
def show_public():
    ...
    with st.sidebar:
        ...
        sel_region = st.selectbox("📍 District", regions)
    ...
    Sidebar se district select karo aur live data dekho.
    </div></div>
    """, unsafe_allow_html=True)

    n = len(dff)
    tl = dff["Loss_Amount"].sum()
    ...
    c1.metric("🗂️ Total Cases", n)
    c2.metric("💸 Total Loss", f"₹{tl/100000:.1f}L")
    c3.metric("📶 UPI Phishing", f"{ur}%")
    c4.metric("💰 Avg Loss", f"₹{al:,}")
    ...
    with tab1:
        ...
        with l:
            ...
            fig.update_xaxes(showgrid=False); fig.update_yaxes(gridcolor="#0a1f35")
            fig.update_layout(showlegend=False)
            dark(fig,"FRAUD CATEGORY — CASE COUNT"); st.plotly_chart(fig,use_container_width=True)
        with r:
            ...
            dark(fig2,"GEOGRAPHICAL LOSS BREAKDOWN"); st.plotly_chart(fig2,use_container_width=True)

        l2,r2 = st.columns(2)
        with l2:
            lf = dff.groupby("Fraud_Type")["Loss_Amount"].sum().reset_index().sort_values("Loss_Amount")
            lf.columns = ["Fraud_Type","Loss"]
            fig3 = px.bar(lf,x="Loss",y="Fraud_Type",orientation="h",color="Loss",
            ...
            dark(fig3,"TOTAL LOSS BY FRAUD TYPE"); st.plotly_chart(fig3,use_container_width=True)
        with r2:
    ...
    with tab3:
        d2 = dff.copy()
        d2["MonthSort"] = pd.to_datetime(d2["Incident_Date"]).dt.to_period("M").apply(lambda p:p.start_time)
        mo = d2.groupby(["MonthSort","Fraud_Type"]).agg(Cases=("Crime_ID","count")).reset_index().sort_values("MonthSort")
        fig7 = px.line(mo,x="MonthSort",y="Cases",color="Fraud_Type",markers=True,color_discrete_sequence=PAL)
        fig7.update_xaxes(gridcolor="#0a1f35"); fig7.update_yaxes(gridcolor="#0a1f35")
        dark(fig7,"MONTHLY FRAUD TREND"); st.plotly_chart(fig7,use_container_width=True)
        ml = d2.groupby("MonthSort")["Loss_Amount"].sum().reset_index().sort_values("MonthSort")
        ...
        dark(fig8,"MONTHLY TOTAL LOSS (₹)"); st.plotly_chart(fig8,use_container_width=True)
